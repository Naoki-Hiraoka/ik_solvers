...
namespace IK{
  VclipLinkPair::VclipLinkPair(const cnoid::Link* link0, Vclip::Polyhedron* vclip_model0, const cnoid::Link* link1, Vclip::Polyhedron* vclip_model1, double tolerance)
  {
    links_[0] = link0;
    links_[1] = link1;
  ...
  double VclipLinkPair::computeDistance(double *q1, double *q2)
  {
    ...
    const cnoid::Vector3& p1 = links_[0]->p();
    cnoid::Matrix3 r1 = links_[0]->attitude();
    const cnoid::Vector3&  p2 = links_[1]->p();
    cnoid::Matrix3 r2 = links_[1]->attitude();
